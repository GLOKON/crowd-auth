...
class CrowdAPI {
	...
	{
		$crowdURL = \Config::get('crowdauth.url');
		$crowdAppName = \Config::get('crowdauth.app_name');
		$crowdAppPassword = \Config::get('crowdauth.app_password');
	...
	public function canUserLogin($username)
	{
		...
		if(count($userGroups) > 0) {
			if(count(array_intersect($userGroups, \Config::get('crowdauth.app_groups'))) > 0) {
				return true;
			}
		}
		return false;
